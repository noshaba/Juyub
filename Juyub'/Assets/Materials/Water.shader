	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				half depth = LinearEyeDepth(_CameraDepthTexture.Sample(sampler_CameraDepthTexture, i.scrPos.xy / i.scrPos.w).r, _ZBufferParams);
				half4 foamLine = 1 - saturate(_Foam * (depth - i.scrPos.w));// foam line by comparing depth and screenposition
				col += foamLine * _Tint; // add the foam line and tint to the texture
				return half4(MixFog(col, i.fogCoords), col.a);
			}
			ENDHLSL
		}
	}
